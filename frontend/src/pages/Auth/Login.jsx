        ...
        <div className="min-h-screen flex items-center justify-center bg-black relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/10 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-400 font-light">{t('auth.login.subtitle')}</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="flex justify-end">
                        <Link to="/forgot-password" className="text-sm text-ocean-400 hover:text-ocean-200 font-medium transition-colors">{t('auth.login.forgotPassword')}</Link>
                    </div>
                ...
                <div className="mt-8 text-center text-gray-400 text-sm">
                    {t('auth.login.noAccount')} <Link to="/register" className="text-ocean-400 font-bold hover:text-ocean-200 ml-1">{t('auth.login.createOne')}</Link>
                </div>
            </div>
        </div>
    );
