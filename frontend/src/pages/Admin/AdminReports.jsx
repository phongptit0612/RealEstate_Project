...
export default function AdminReports() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-900">{t('admin.reports', 'Reports')}</h1>
                <p className="text-slate-500 mt-1">{total} {t('admin.reportsCount', 'reports in this category')}</p>
            </div>

            <div className="flex gap-2 bg-slate-100 p-1 rounded-xl w-fit">
                {STATUS_TABS.map(tabKey => (
                    <button key={tabKey} onClick={() => setTab(tabKey)}
                        className={`px-4 py-2 rounded-lg text-sm font-semibold capitalize transition-all ${tab === tabKey ? 'bg-white shadow text-brand-600' : 'text-slate-500 hover:text-slate-700'}`}>
                        {tabKey === 'pending' ? t('common.pending', 'Pending') : tabKey === 'reviewed' ? t('admin.reviewed', 'Reviewed') : t('admin.dismissed', 'Dismissed')}
                    </button>
                ))}
            </div>
            ...
            <div className="space-y-4">
                {loading ? (
                    ...
                    <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm">
                        ...
                        <p className="text-slate-400">{t('admin.noReports', 'No reports in this category.')}</p>
                    </div>
                ) : reports.map(r => (
                            ...
                            <div className="space-y-1 flex-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <span className="text-xs bg-red-100 text-red-600 font-bold px-2.5 py-1 rounded-full uppercase">
                                        {t('detail.' + r.reason, r.reason || t('detail.other', 'Other'))}
                                    </span>
                                    <span className="text-xs text-slate-400">{new Date(r.created_at).toLocaleDateString()}</span>
                                </div>
                                <p className="font-semibold text-slate-800">{t('admin.listings', 'Listing')}: {r.property_title}</p>
                                <p className="text-sm text-slate-600">{t('admin.reporter', 'Reporter')}: <span className="font-medium">{r.reporter_name}</span> — {r.reporter_email}</p>
                                {r.details && (
                            ...
                            {tab === 'pending' && (
                                <div className="flex gap-2 flex-shrink-0">
                                    <button onClick={() => updateStatus(r.report_id, 'reviewed')}
                                        ...
                                        <CheckCircle className="w-4 h-4" /> {t('admin.reviewed', 'Reviewed')}
                                    </button>
                                    <button onClick={() => updateStatus(r.report_id, 'dismissed')}
                                        ...
                                        <XCircle className="w-4 h-4" /> {t('admin.dismiss', 'Dismiss')}
                                    </button>
                                </div>
                            )}
                        </div>
