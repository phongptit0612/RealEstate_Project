...
export default function Pricing() {
    ...
    const { formatPrice, convertPrice, exchangeRates, preferredCurrency, setCurrency } = useCurrencyStore();
    ...
    // Format price in the selected display currency
    const displayPrice = (usd) => {
        ...
        if (selectedCurrency === 'VND') {
            converted = Math.round(usd * rate);
        } else if (selectedCurrency === 'USD') {
            converted = usd;
        } else {
            // EUR: rate_to_usd = how many USD 1 EUR buys
            converted = usd / rate;
        }
            ...
            <div className="max-w-5xl mx-auto px-6 pt-12 pb-4">
                ...
                <div className="text-center mb-12">
                    <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-400/20 px-4 py-1.5 rounded-full text-amber-400 text-sm font-semibold mb-6">
                        <Crown className="w-4 h-4" /> VIP Listing Boosts
                    </div>
                    <h1 className="text-5xl font-extrabold text-white tracking-tight mb-4">
                        Supercharge Your Listing
                    </h1>
                    <p className="text-slate-400 text-lg max-w-xl mx-auto">
                        Stand out from the crowd. VIP listings get more eyes, more clicks, and sell faster.
                    </p>
                </div>
                ...
                {(!hasStripeKey) && (
                    <div className="text-center text-amber-400 bg-amber-500/10 border border-amber-400/20 rounded-xl px-4 py-3 mb-6 text-sm">
                        ⚡ <strong>Dev Mode Active</strong> — No Stripe key detected. Payments will be simulated instantly.
                    ...
                    {Object.values(TIERS).map(tier => (
                        ...
                        >
                            ...
                            <div className="mb-6">
                                ...
                                <span className="text-slate-400 text-sm ml-2">/ 30 days</span>
                            </div>
                            ...
                            >
                                {loading === tier.id
                                    ...
                                    : <><Zap className="w-4 h-4" /> Boost with {tier.name}</>
                                }
                            </button>
                        </div>
                    ))}
